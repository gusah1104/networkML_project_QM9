...
class PlaceHolder:
    ...
    def mask(self, node_mask):
        ...
        e_mask1 = node_mask.unsqueeze(1).unsqueeze(-1)  # bs, 1, n, 1
        e_mask2 = e_mask1.transpose(2,1)  # bs, n, 1, 1
        diag_mask = ~torch.eye(n, dtype=torch.bool, device=node_mask.device).unsqueeze(0).unsqueeze(-1).repeat(bs, 1,1,1) # bs, n, n, 1
